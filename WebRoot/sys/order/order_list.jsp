	...
	<div id="MainArea">
		<table class="MainArea_Content" align="center" cellspacing="0"
			...
			<tbody id="TableData">
        		...
        		<c:when test="${not empty requestScope.pageResult.items }">
        			<c:forEach var="order" items="${requestScope.pageResult.items }" varStatus="o">
        				<tr align="center">
							...
							<td><a href="${basePath}sys/order_detail.action?order.id=${order.id}" class="FunctionButton">详细</a>

							<a href="${basePath}sys/order_pay.action?order.id=${order.id}" class="FunctionButton">结账</a>
							<a href="${basePath}sys/order_delete.action?order.id=${order.id}" class="FunctionButton">删除</a>
							</td>
						</tr>      			
        			</c:forEach>
        		</c:when>
        		<c:otherwise>
        			<tr>
        				<td colspan="7">没有你要找找的数据，请先保存记录再查看！</td>
        			</tr>
        		</c:otherwise>
        	</c:choose>
