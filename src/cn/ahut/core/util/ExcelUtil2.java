		...
		try {
			...
			if(userList != null){
				for(int j = 0; j < userList.size(); j++){
					...
					cell12.setCellValue(userList.get(j).getRole());
					HSSFCell cell13 = row.createCell(2);
					cell13.setCellValue(userList.get(j).getMobile());
					HSSFCell cell14 = row.createCell(3);
					cell14.setCellValue(userList.get(j).getAddress());
				}
			}
			//5、输出
